-- f7bf6b41a6c0fda7a9c8c0cf00e0f611 http://api.2445579939911.proxy.3scale.net:80 http://ec2-54-242-30-126.compute-1.amazonaws.com:3000 1006371746481 none --
-- -*- mode: lua; -*-
-- Generated on: Tue Nov 13 11:32:59 +0000 2012 --
-- Version:
-- Error Messages per service

      service_1006371746481 = {
        error_auth_failed = 'Authentication failed',
        error_over_limit = 'Limits exceeded',
        error_auth_missing = 'Authentication parameters missing',
        auth_failed_headers = 'text/plain; charset=us-ascii',
        auth_missing_headers = 'text/plain; charset=us-ascii',
        over_limit_headers = 'text/plain; charset=us-ascii',
        error_no_match = 'No rule matched',
        no_match_headers = 'text/plain; charset=us-ascii',
        no_match_status = 404,
        auth_failed_status = 403,
        over_limit_status = 409,
...
function error_over_limit(service)
   ngx.status = service.over_limit_status
   ngx.header.content_type = service.over_limit_headers
   ngx.print(service.error_over_limit)
   ngx.exit(ngx.HTTP_OK)
end
...
matched_rules2 = ""
        ...
        function extract_usage_1006371746481(request)
...
function get_debug_value()
   ...
   if h["X-3scale-debug"] == 'f7bf6b41a6c0fda7a9c8c0cf00e0f611' then
      return true
   else
      return false
   end
...
local service = {}
      if ngx.var.service_id == '1006371746481' then
        local parameters = get_auth_params("not_headers", string.split(ngx.var.request, " ")[1] )
        service = service_1006371746481 --
        ngx.var.secret_token = service.secret_token
              params.app_id = parameters["app_id"]
        params.app_key = parameters["app_key"]
        get_credentials_app_id_app_key(params, service_1006371746481)
        ngx.var.cached_key = "1006371746481" .. ":" .. params.app_id ..":".. params.app_key
              ngx.var.service_id = "1006371746481"
        ngx.var.proxy_pass = "http://backend_ec2-54-242-30-126.compute-1.amazonaws.com"
        ngx.var.usage = extract_usage_1006371746481(ngx.var.request)
      end
